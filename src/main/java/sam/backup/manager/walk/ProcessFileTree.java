...
public class ProcessFileTree implements FileTreeWalker {
	...
	private final List<FileImpl> willRemoved = new ArrayList<>();
	...
	public ProcessFileTree(Config config, boolean backupWalked) {
		...
		willRemoved.forEach(FileImpl::remove);
	}
	...
	private void isBackable(FileImpl ft, boolean isfile) {
	...
	private boolean check(FileImpl f, boolean condition, String reason) {
		if(condition)
